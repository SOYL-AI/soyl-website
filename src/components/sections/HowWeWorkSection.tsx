  ...
  return (
    <section className="bg-soyl-white py-16 md:py-24">
      <div className="max-w-content mx-auto px-6 lg:px-16">
        <SectionLabel className="text-obsidian/60">How We Work</SectionLabel>
        <h2 className="font-heading text-3xl md:text-4xl font-bold text-obsidian mb-12">
          Our Process
        </h2>
          ...
          <div className="space-y-8">
            {STEPS.map((step, i) => (
              <div key={step.number} className="flex gap-6 items-start">
                <span className={`font-display font-bold text-4xl ${i === 0 ? 'text-mint' : 'text-obsidian/20'}`}>
                  {step.number}
                </span>
                <div>
                  <h3 className={`font-heading font-bold text-lg mb-1 ${i === 0 ? 'text-obsidian' : 'text-obsidian/70'}`}>
                    {step.title}
                  </h3>
            ...
            <a href="/how-we-work" className="text-obsidian text-sm font-medium hover:text-mint transition-colors inline-block mt-4">
              See Full Process →
            </a>
          </div>

          {/* Right — Visual placeholder */}
          <div className="aspect-video bg-obsidian/10 rounded-xl flex items-center justify-center">
        ...
        <div className="overflow-hidden mt-16">
          <div className="flex gap-4 animate-marquee hover:[animation-play-state:paused]">
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="flex-shrink-0 w-64 h-40 rounded-xl bg-obsidian/10" />
            ))}
          </div>
        </div>
      </div>
